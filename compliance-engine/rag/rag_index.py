...
-----
    ...
    python -m rag.rag_index --input data/mabhas_clauses.json --dry-run

    # full ingest:
    DATABASE_URL=postgresql://user:pass@localhost:5432/compliance \
        python -m rag.rag_index --input data/mabhas_clauses.json
...
def _build_passage_text(clause: Dict[str, Any]) -> str:
    """Build the text that gets embedded for one clause.

    Stage 1 / Step 2: prefer the Stage 0 normalized Persian fields
    (Arabic ي/ك → Persian ی/ک, unified digits, momayyez handling) and fall
    back to the raw fields so pre-Stage-0 JSON files still ingest.
    English text is appended unchanged, as before.

    Stage 1 / Step 5: when the clause carries an LLM-generated context_fa,
    it is PREPENDED, so the passage becomes
        context_fa + heading + Persian body + text_en.
    Files without context_fa (e.g. mabhas_clauses_normalized.json) still
    ingest identically to Step 2.
    """
    heading = clause.get("heading_fa_normalized") or clause.get("heading_fa")
    body = clause.get("text_fa_normalized") or clause["text_fa"]

    parts: List[str] = []
    if clause.get("context_fa"):
        parts.append(str(clause["context_fa"]).strip())
    if heading:
...
def ingest(
    ...
    passages = [_build_passage_text(c) for c in keep]  # also stored (Step 4)
    from rag.embeddings import MODEL_NAME as _embed_model
    print(f"Embedding passages with {_embed_model} "
          "(first run downloads the ~2 GB model)...")
    ...
    rows = []
    for clause, emb, passage in zip(keep, embeddings, passages):
        rows.append((
    ...
    upsert_sql = """
        ...
        ON CONFLICT (mabhas_part, article_id) DO UPDATE SET
            ...
            context_fa               = EXCLUDED.context_fa;
    """
